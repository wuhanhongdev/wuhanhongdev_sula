...
//	var _checkrebut = function(id){};
	...
	//获取对象
	var _getConsult = function(id){
		$http({
			method : 'POST',
			url : 'subinfo/getConsultById',
			data : {'id':id},
			headers: {  
		        'Content-Type': 'application/x-www-form-urlencoded'  
		    },  
	...
	$scope.submitAdvert = function(){
		...
		if(data.id == null){
			_save(data,'subinfo/addConsult');
		}else{
			_save(data,'subinfo/updateConsult');
		}
	};
